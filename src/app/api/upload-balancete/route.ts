  ...
  try {
    ...
    await dbQuery(
      `
        ...
        VALUES ($1, 'balancete', 'processing', 0, 0, 0, 'Preparando arquivo...', NULL)
        ON CONFLICT (job_id)
        DO UPDATE SET
    ...
    const referenceDate =
      uploadedDates.length > 0
        ? uploadedDates.reduce((currentMax, currentValue) =>
    ...
    try {
      ...
      const deleteResult = importMode === "full"
        ? await client.query("DELETE FROM balancete")
        : await client.query(
            `
              DELETE FROM balancete
      ...
      for (let start = 0; start < rows.length; start += insertChunkSize) {
        ...
        await client.query(
          `
            INSERT INTO balancete (conta, id1, id2, id3, data, valor)
            VALUES ${placeholders}
          `,
          values,
        );
      ...
      await updateUploadJob(jobId, {
        ...
        error: error instanceof Error ? error.message : "Erro ao importar Balancete.",
      });
      throw error;
    } finally {
      client.release();
    ...
    if (jobId) {
      await updateUploadJob(jobId, {
        status: "error",
        message: "Falha durante a importação.",
        error: error instanceof Error ? error.message : "Erro ao processar Balancete.",
      });
    }
